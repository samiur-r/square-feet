...
const ArchivedPosts: NextPage = () => {
  ...
  useEffect(() => {
    setPosts(filteredArchivedPosts)
    setPostCount(filteredArchivedPosts.length)
  }, [filteredArchivedPosts])

  useEffect(() => {
    setTotalPosts(totalFilteredArchivedPosts)
  }, [totalFilteredArchivedPosts])
    ...
    try {
      const response = await ApiClient({
        ...
        data: {
          limit,
          offset,
          location: archivedLocationsSelected,
          propertyType: archivedPropertyTypeSelected,
          category: archivedCategorySelected
        }
      })
      ...
      updateFilteredArchivedPosts(
        resetFilteredPosts
          ? response.data.posts
          : [...filteredArchivedPosts, ...response.data.posts]
      )
      updateCanFetchArchivedPosts(false)
    } catch (error) {
      setIsCallingApi(false)
    }
  }
  ...
  useEffect(() => {
    if (isIntersecting && !canFetchArchivedPosts) {
  ...
  const breadcrumbsItems = [
    ...
    {
      title: `عقارات ${archivedCategorySelected?.title} في الكويت`,
      href: `${archivedCategorySelected?.title}`
    },
    archivedPropertyTypeSelected && archivedPropertyTypeSelected.id !== 0
      ? {
          title: `${archivedPropertyTypeSelected?.title} ${archivedCategorySelected?.title}`,
          href: `${archivedPropertyTypeSelected?.title}/${archivedCategorySelected?.title}`
        }
      : null
  ]
  ...
  const breadcrumbsItemsWithLocation = [
    ...
    {
      title: `${archivedCategorySelected?.title} في ${getStateTitleFromCity(
        archivedLocationsSelected[0]
      )}`,
      href: `${getStateTitleFromCity(archivedLocationsSelected[0])}/${
        archivedCategorySelected?.title
      }`
    },
    archivedLocationsSelected &&
    archivedLocationsSelected.length &&
    archivedLocationsSelected[0]?.state_id !== null
      ? {
          title: `${archivedCategorySelected?.title} في ${archivedLocationsSelected[0]?.title}`,
          href: `${archivedLocationsSelected[0]?.title}/${archivedCategorySelected?.title}`
        }
      : null,
    archivedLocationsSelected &&
    archivedLocationsSelected.length &&
    ...
    archivedPropertyTypeSelected
      ? {
          title: `${archivedPropertyTypeSelected?.title} ${archivedCategorySelected?.title} في ${archivedLocationsSelected[0]?.title}`,
          href: `${archivedLocationsSelected[0]?.title}/${archivedPropertyTypeSelected?.title}/${archivedCategorySelected?.title}`
        }
      : null
  ]
  ...
  return (
    <div>
      <Breadcrumbs
        breadcrumbsItems={
          archivedLocationsSelected && archivedLocationsSelected.length
